...
DECLARE @SprocPrifix varchar(3)
SET @SprocPrifix = ''

DECLARE @IsSoftDelete bit
SET @IsSoftDelete = 1
	...
	IF @TableName <> @CurrentTable BEGIN
		...
		IF @CurrentTable <> '' BEGIN
			IF @GenerateProcsFor = '' OR @GenerateProcsFor = @CurrentTable BEGIN
				...
				SET @LIST = @LIST + CHAR(13) + 'Go' + CHAR(13)
				...
				SET @SELECT = @SELECT + CHAR(13) + 'Go' + CHAR(13)
				...
				SET @UPSERT = @UPSERT + CHAR(13) + 'Go' + CHAR(13)
		...
		IF @GenerateProcsFor = '' OR @GenerateProcsFor = @CurrentTable BEGIN
			...
			-- _sel
			SET @SELECT = 'CREATE PROC [dbo].[' + @SprocPrifix + @ObjectName + '_sel]' + Char(13)
			...
			-- _ups
			SET @UPSERT = 'CREATE PROC [dbo].[' + @SprocPrifix + @ObjectName + '_ups]' + Char(13)
			...
			-- _del
			SET @DELETE = 'CREATE PROC [dbo].[' + @SprocPrifix + @ObjectName + '_del]' + Char(13)
